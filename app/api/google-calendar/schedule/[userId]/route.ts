...
async function createCalendarEvent({ summary, description, startISO, endISO, origin, calendarId, userId }: { summary: string; description: string; startISO: string; endISO: string; origin: string; calendarId?: string; userId: string }) {
  console.log("📅 createCalendarEvent started:", { summary, startISO, endISO, origin, userId })
  try {
    const calendar = await getCalendarFromClerk(origin, userId)
    console.log("📅 Calendar client initialized for user:", userId)
    ...
    console.log("✅ Google Calendar Response:", JSON.stringify(res.data, null, 2))
    return res.data
  } catch (err) {
...
async function appendBookingRow({
  ...
  const sheets = await getSheetsFromClerk(origin, userId)
...
export async function POST(
  ...
  try {
    ...
    const defaultSpreadsheetId = (await (await import('@/lib/google-from-clerk')).getUserSpreadsheetId(userId))
    const spreadsheetIdToUse = body?.spreadsheetId || defaultSpreadsheetId
    if (spreadsheetIdToUse) {
      await appendBookingRow({
        spreadsheetId: spreadsheetIdToUse,
